...
`POST /v1/catalogs/products` (un prodotto, "Tessera") poi `POST /v1/billing/plans` — due chiamate per ciascun piano a pagamento (oggi solo Plus, Free non ha un piano PayPal), una per ciclo mensile e una per ciclo annuale (docs/13-piano-miglioramenti.md, D2): PayPal non ha un piano che fattura a due frequenze diverse, quindi sono due risorse distinte. Ognuna include un billing cycle `TRIAL` a costo zero (`BillingDefaults.TrialDays`, 14 giorni placeholder) anteposto a quello `REGULAR`. Fatto automaticamente all'avvio (`PayPalSubscriptionService.EnsurePlansProvisionedAsync`, idempotente — provisiona indipendentemente ciascuna delle quattro colonne id ancora nulle), non serve setup manuale. L'id restituito va in `SubscriptionPlan.PayPalPlanIdSandbox`/`...IdLive` (mensile) o `...SandboxAnnual`/`...LiveAnnual` (annuale) a seconda di `PayPal:Environment` — vedi [02-modello-dati.md](02-modello-dati.md#abbonamento-paypal-per-spazio) per il perché delle colonne separate. Il prodotto stesso non ha questo problema: sandbox e live sono account PayPal completamente separati, quindi `EnsureProductAsync` (che cerca per nome invece di salvare l'id) non rischia mai di trovare il prodotto dell'altro ambiente.

### Flusso di sottoscrizione

1. Console: l'utente sceglie un piano e un ciclo (mensile/annuale, selettore su `/pricing` e `/spaces/{id}/usage`) → `POST /v1/billing/subscriptions` con il `PayPalPlanId` del ciclo scelto → la risposta contiene un link HATEOAS `rel: "approve"`. Il ciclo scelto viene salvato su `SpaceSubscription.BillingCycle`, così un cambio di solo piano tiene lo stesso ciclo senza richiederlo di nuovo.
...
La dettatura vocale nativa di Telegram e WhatsApp sul telefono copriva gran parte del bisogno a costo zero — ma richiede comunque aprire l'app e guardare la tastiera per confermare il testo dettato, non proprio "mentre guido". **Implementato** (docs/13-piano-miglioramenti.md, E1): il messaggio vocale nativo di Telegram viene trascritto lato server (`VoiceTranscriptionClient`, `Tessera.Ai`, stesso pattern di `ReceiptVisionClient` — un client LLM dedicato che inghiotte i propri errori e torna `null`) e il trascritto rientra in `ProcessAsync` esattamente come se fosse stato digitato, tramite lo stesso meccanismo di *replay* già usato per `HandleSpaceChoiceCallbackAsync`/`HandlePermissionFallbackCallbackAsync` (`ProviderMessageId` prefissato `replay:`, mai reinserito in `ProcessedMessage`). Un vocale passa quindi per L2 come il testo — mai un salto diretto a L3 — ed è soggetto alla stessa soglia giornaliera `UsageService.TryRecordL3CallAsync`, addebitata prima della trascrizione stessa (il costo reale è lì, non nel routing), più un limite di durata (60 secondi, guardia di costo non di protocollo). Deployment Azure OpenAI separato da quello di chat/vision (`AzureOpenAI:TranscriptionDeployment`, [08-setup-sviluppo.md](08-setup-sviluppo.md)) — un modello di trascrizione non è lo stesso deployment gpt-4o-mini usato altrove. Se in futuro emerge che l'Echo in cucina è il punto d'ingresso principale dell'utenza, la custom skill Alexa resta comunque fuori roadmap come MVP, riconsiderabile solo in Fase 5.
