    ...
    LlmFallbackClient? llmFallback = null) : BackgroundService
{
    ...
    private async Task ProcessAsync(InboundMessage message, CancellationToken ct)
    {
        ...
        if (message.CallbackData is { } callbackData)
        {
            ...
            await HandleCallbackAsync(shoppingHandlers, shopping, expenses, reminders, budgets, notifications, undo, onboarding, address, spaceId, user, culture, callbackData, message.CallbackMessageId, ct);
            return;
        }
        ...
        else
        {
            ...
            reply = await HandleLlmFallbackAsync(
                scope, shoppingHandlers, shopping, expenses, reminders, notes, budgets, notifications, undo, onboarding, address, spaceId, user, culture, text, ct);
        }
    ...
    private async Task<string?> HandleLlmFallbackAsync(
        AsyncServiceScope scope, ShoppingHandlers shoppingHandlers, ShoppingListService shopping, ExpenseService expenses, ReminderService reminders,
    ...
    private async Task HandleCallbackAsync(
        ShoppingHandlers shoppingHandlers, ShoppingListService shopping, ExpenseService expenses, ReminderService reminders, BudgetService budgets,
