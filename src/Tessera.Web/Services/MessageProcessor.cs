    ...
    LlmFallbackClient? llmFallback = null) : BackgroundService
{
    ...
    private async Task ProcessAsync(InboundMessage message, CancellationToken ct)
    {
        ...
        else
        {
            ...
            reply = await HandleLlmFallbackAsync(
                scope, shoppingHandlers, expenseHandlers, shopping, expenses, reminders, notes, budgets, notifications, undo, onboarding, address, spaceId, user, culture, text, ct);
        }
    ...
    private async Task<string?> HandleLlmFallbackAsync(
        AsyncServiceScope scope, ShoppingHandlers shoppingHandlers, ExpenseHandlers expenseHandlers, ShoppingListService shopping, ExpenseService expenses, ReminderService reminders,
